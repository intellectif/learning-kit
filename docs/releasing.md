...
The App must belong to the **organization** that owns this repository, `intellectif`. One created
under a personal account can only be installed there: the token step then fails with `Not Found`
on `GET /repos/intellectif/learning-kit/installation`. (An App created in the wrong place can be
moved: its page → Advanced → Transfer ownership → `intellectif`; its Client ID and key stay valid.)

1. **Create it in the organization** — your profile picture → **Your organizations** → **Settings**
   next to `intellectif` → **Developer settings** → **GitHub Apps** → **New GitHub App**.
   - **GitHub App name:** anything unused, e.g. `learning-kit-release`.
   - **Homepage URL:** `https://github.com/intellectif/learning-kit`.
   - **Callback URL:** empty; leave "Request user authorization (OAuth) during installation" off.
   - **Webhook:** untick **Active**.
   - **Repository permissions:** **Contents: Read and write** and **Pull requests: Read and
     write**; Metadata is Read-only and mandatory. Everything else, and every organization and
     account permission, stays at *No access* — not Workflows: the version PR never edits them.
   - **Where can this GitHub App be installed?** *Only on this account*.
   - **Create GitHub App**.
2. **Note the Client ID** (it starts with `Iv`; not the numeric App ID) on the App's page.
3. **Make a private key** — same page, **Private keys** → **Generate a private key**. A `.pem`
   file downloads. A *client secret* is something else — for signing people in, which this App
   never does — and is not used; one generated by mistake is harmless left unused.
4. **Install it** — the App's **Install App** → **Install** next to `intellectif` → **Only select
   repositories** → `learning-kit` → **Install**.
5. **Give them to the workflow** — this repository's **Settings → Secrets and variables →
   Actions**: under **Variables**, `RELEASE_APP_CLIENT_ID` = the Client ID; under **Secrets**,
   `RELEASE_APP_PRIVATE_KEY` = the whole `.pem` file, from `-----BEGIN` to `-----END` inclusive.
   Then delete the downloaded file; a new key can be generated any time.
6. **Check it** — the next Release run's "Token for the release PR" step succeeds instead of being
   skipped, the version PR is opened by the App (`<name>[bot]`), and its CI and E2E start without
   an approval. A failed token step publishes nothing: fix the variable or secret and use
   **Re-run jobs**.
...
> pinned exactly — Changesets 3 drops it — and why `release.yml` stays on `changesets/action` v1,
> the line for Changesets 2 (v2 is built for Changesets 3 and ignores this workflow's inputs and
> token). `.github/dependabot.yml` holds both majors back; they move together, as a change of
> their own. Decide *before* merging — npm versions are immutable.
