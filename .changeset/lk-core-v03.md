...
v0.3.0 — "close the holes, open the type system". Every change that touches grading semantics is opt-in: for identical `(data, response)` inputs, `score` / `passed` are bit-for-bit v0.2. Runtime-additive throughout; three TypeScript-surface notes for upgrading consumers:
...
**New: `redact()` + `FieldPolicy` + `assertRedacted()` (R7).** Fail-closed, exhaustive redaction driven by per-type field policies: unclassified fields (including unknown passthrough keys) are never emitted; `scoringStrategy`, answer keys, and authored feedback are stripped; rubrics are author-only. Default (`reveal: 'none'`) output validates against new strict redacted schemas (`RedactedMultipleChoiceDataSchema` etc.); `reveal: 'after-submit'` restores answer-key fields for review renders.
...
**Consumer migration (academic-platform-ocw):** replace the app-side `written-response.ts` shim with SDK imports; delete the raw-payload merge workaround around `validateActivity` (unknown keys now survive); replace the hand-rolled redactor with `redact()`; import `DEFAULT_PASS_THRESHOLD` instead of duplicating the constant. No DB migration required — the wire format is field-for-field identical.
