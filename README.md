...
`lk-react` declares `lk-core` as a **peer** dependency. Since 1.0, a `lk-core` minor or patch releases no `lk-react`, so the two version numbers move apart; `lk-react` is at 23.x because, before 1.0, every `lk-core` minor forced a `lk-react` major ([Stability](./docs/stability.md#the-two-version-numbers)). A `lk-react` major from those days is therefore not by itself evidence of a breaking React API — check its [CHANGELOG](./packages/lk-react/CHANGELOG.md).
...
- **Authoring contracts** *(v0.9)* — `validateDraft` tells an unfinished question from a wrong one, per activity type, with a documented issue code for every problem it recognises; `createDraft` starts a new one; `<ActivityPreview>` previews a draft in any mode with a simulated answer. From v1.2, an item critic points out what a reviewer would on a valid item — the right option much longer, an answer printed in the passage — and, with your model, a review of an item and drafts from a passage, a script or a video's captions — the types the author ticks, in the author's order, placed on the video where their captions end — each checked and never approved by the SDK. The editor itself stays yours.
