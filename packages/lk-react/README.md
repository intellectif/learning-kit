...
- **`<LkIntlProvider>`** — all 167 strings the SDK's own chrome renders, replaceable in one place, with a per-component `strings` prop for the exceptions. Interpolation and plurals are **functions**, not format strings, so your `Intl.PluralRules` does the work and TypeScript checks the arity. `locale` sets `lang` and derives `dir`; the skin uses logical properties, so RTL follows. The SDK ships the mechanism and **English only** — see [docs/i18n.md](https://github.com/intellectif/learning-kit/blob/main/docs/i18n.md).
...
| `@intellectif/lk-react/components/InteractiveVideo` | `<InteractiveVideo>` a video that stops at each quiz (boundary-wrapped), with `InteractiveVideoSlot`, `InteractiveVideoSummary` and the `VideoPreferences` type |
